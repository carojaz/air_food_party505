...
d1 = Device.new(name: "Raclette vintage", description: "LA plus belle des raclettes à votre table!", price: 500)
d1.user_id = u1.id
d1.category_id = c1.id
d1.save!
d2 = Device.new(name: "Pierrade de ouf", description: "La meilleure viande grillée du monde", price: 400)
d2.user_id = u2.id
d2.category_id = c2.id
d2.save!
d3 = Device.new(name: "Crêpe de dingue", description: "Votre meilleure crêpe!...surtout avec du Nutella!", price: 700.50)
d3.user_id = u3.id
d3.category_id = c3.id
d3.save!
d4 = Device.new(name: "PopCorn tous ensemble", description: "Anti-covid, mangez des pop corn tous ensemble!!", price: 200)
d4.user_id = u3.id
